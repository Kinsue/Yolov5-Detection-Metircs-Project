...
def main(opt):
    ...
    for si in range(len(list(det_path.glob('*.txt')))):
        ...
        correct = process_batch(torch.tensor(predn, device=device),
        ...
        stats.append((correct,
                      ...
                      torch.tensor(labels[:, 0], device=device)))  # (correct, conf, pcls, tcls)
    ...
    if nt.sum() == 0:
        print(f'WARNING: no labels found in dataset, can not compute metrics without labels ⚠️')
...
def parse():
    ...
    parser.add_argument('-det', '--detection-path', default=ROOT / 'input/DR', help="Detection result path.")
    parser.add_argument('-gt', '--groundtruth-path', default=ROOT / 'input/GT', help="GroundTruth path.")
    parser.add_argument('-img', '--images', default=ROOT / 'input/image', help="images path ")
    parser.add_argument('-l', '--labels', default=ROOT / 'input/label.txt', help="label path")
